...
var bos = "Aranıyor..."
    ...
    let store = args.slice(0).join('+');
        ...
        if(!store)return message.channel.send("Hangi Öğeyi Aratmak istersin. **Örnk Kullanım:** `playstore Total Destraction`")
        if(!link)return message.channel.send("Öğe Bulunamadı.")
